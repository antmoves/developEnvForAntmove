...
UPSTREAM="https://github.com/antmoves/developEnvForAntmove.git"
...
register() {
    ...
    [ -d "$CONFIG_PATH" ] ||
        last_advice "Configuration files were not generated, $fix_advice"
    ...
    [ ${#cfg_files[@]} -ne "$(echo -e "$USER" | wc -l)" ] &&
        last_advice "Some accounts failed to register its app, $fix_advice"
...
invoke() {
    [ -d "$CONFIG_PATH" ] ||
        last_advice "没有找到配置文件, 请执行应用注册 Action." "o"
